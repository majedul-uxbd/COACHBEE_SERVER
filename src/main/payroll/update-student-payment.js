...
const updateStudentPaymentDataQuery = async (paymentData) => {
    const _query = `
        UPDATE
        ...
        SET
            ...
            ${TABLE_STUDENT_PAYMENTS_COLUMNS_NAME.UPDATED_AT} = ?
        WHERE
...
/**
 * This function updates a student payment record in the database based on the provided payment data. 
 * It validates the input data, checks if the student exists and is active, and then updates the payment record in the database. 
 * 
...
const updateStudentsPayment = async (lg, authData, paymentData) => {
    ...
    try {
        ...
        const isUpdated = await updateStudentPaymentDataQuery(paymentData);
        if (isUpdated) {
            return Promise.resolve(
                setServerResponse(
                    API_STATUS_CODE.OK,
                    'student_payment_updated_successfully',
                    lg || 'en',
                )
            )
        }
