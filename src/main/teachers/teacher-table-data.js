 ...
 */
const totalTeachersTableRowCount = async (authData) => {
    const query = `
    SELECT
        COUNT(*) AS totalRows
    FROM
...
const getTeachersDetailsDataQuery = async (authData, paginationData) => {
    const _query = `
    SELECT
    ...
    WHERE
        ${TABLE_TEACHERS_COLUMNS_NAME.UUID} = ?
    ORDER BY
            ${TABLE_TEACHERS_COLUMNS_NAME.CREATED_AT} ${paginationData.sortOrder}
        LIMIT ? OFFSET ?;
    `;
    const _values = [
        authData.uuid,
